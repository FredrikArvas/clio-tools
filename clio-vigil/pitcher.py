...
def _send_pitch(journalist: dict, pitch_text: str, from_account: str) -> bool:
    """Skickar pitch via clio-agent-mail SSH. Returnerar True om lyckades."""
    import subprocess
    import shlex
    ...
    # Extrahera ämnesrad från pitch_text (första raden efter "Ämnesrad:")
    subject = ""
    ...
    cmd = [
        "ssh", "clioadmin@100.107.127.104",
        f"cd ~/18.0/clio-tools/clio-agent-mail && python3 -c \""
        f"import sys; sys.path.insert(0,'.')\n"
        f"from smtp_client import send_email\n"
        f"import configparser, os\n"
        f"from pathlib import Path\n"
        f"from dotenv import load_dotenv\n"
        f"load_dotenv(Path('.').parent/'.env'); load_dotenv(Path('.')/ '.env', override=True)\n"
        f"config = configparser.ConfigParser(interpolation=None)\n"
        f"config.read('clio.config', encoding='utf-8')\n"
        f"for k in config.get('mail','accounts').split(','):\n"
        f"    k=k.strip(); v=os.environ.get(f'IMAP_PASSWORD_{{k.upper()}}')\n"
        f"    if v: config.set('mail',f'imap_password_{{k}}',v)\n"
        f"send_email(config=config, from_account_key={shlex.quote(from_account)}, "
        f"to_addr={shlex.quote(email)}, subject={shlex.quote(subject)}, "
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.error("SSH-fel: %s", result.stderr[:200])
            return False
    except Exception as exc:
