"""
mail_service.py — HTTP-wrapper för smtp_client

Exponerar POST /send så att clio-moduler på andra maskiner kan skicka
mail via clio@arvas.international utan egna SMTP-credentials.

Kör: python mail_service.py [--port 7100]

Miljövariabler (root .env + clio-agent-mail/.env):
...
class _Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if not required.issubset(data):
            self._respond(400, {"ok": False, "error": f"saknade fält: {required - set(data)}"})
            return
...
def main():
    ...
    parser = argparse.ArgumentParser(description="clio-mail HTTP-tjänst")
    parser.add_argument(
        "--port", type=int,
