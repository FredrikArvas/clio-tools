...
Körning:
  ...
  python3 ingest_crm.py --file "foo.pdf"       # enskild fil
  python3 ingest_crm.py --force                # tvinga omindexering
"""
...
EMBED_DIM     = 1536
CORPUS_DIR    = _here / "corpus_crm_ssf"
CHUNK_WORDS   = 500
OVERLAP_WORDS = 60
...
def ensure_collection() -> None:
    ...
    if COLLECTION not in existing:
        ...
        print(f"[ingest_crm] Skapade collection: {COLLECTION}")
    else:
        print(f"[ingest_crm] Collection finns: {COLLECTION}")
    ...
    for block in iter_blocks(doc):
        if isinstance(block, Paragraph):
            ...
            for row in block.rows:
                # Dedup merged cells (same object repeated)
                seen, cells = set(), []
...
# ---------------------------------------------------------------------------
def main() -> None:
    parser = argparse.ArgumentParser(description="Indexera SSF CRM 2023-dokument")
    ...
    print(f"\n[ingest_crm] KLAR — {total} nya chunks totalt i '{COLLECTION}'")
