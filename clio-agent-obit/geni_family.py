...
def print_family(result: dict):
    ...
    print(f"  {focus['name']}")
    if focus["birth_date"]:
        print(f"  Fodd: {focus['birth_date']}  {focus['birth_location']}")
    if focus["death_date"]:
        print(f"  Dod:  {focus['death_date']}")
    print(f"{'='*50}")

    for label, people in [
        ("Foraldrar",  result["parents"]),
...
def main(argv=None):
    ...
    if not args:
        print("Anvandning: python geni_family.py <geni-URL-eller-ID>")
        sys.exit(1)

    arg = args[0]
    profile_id = profile_id_from_url(arg) if arg.startswith("http") else arg
    print(f"Hamtar familj for profil-ID: {profile_id}...")
