"""
clio2.py  — AKTIV VERSION (Odoo-migrationsspår, startar 2026-04-25)
Tunn launcher för clio-tools under transitionsperioden mot Odoo.

Verktyg försvinner ur denna meny allteftersom de migreras till Odoo.
Se ADD-odoo-integration.md för migrationsstatus och releaseplan.

Migrationsstatus per verktyg:
  🔜 RELEASE 1  clio-agent-job   → clio_job (Odoo-addon)
  🔜 RELEASE 2  clio-agent-obit  → clio_obit (Odoo-addon)
  🔜 RELEASE 3  clio-vigil       → clio_vigil (Odoo-addon)
  🔧 PLANERAD   clio-graph, clio-agent-odoo, clio-agent-mail
  🖥️ CLI ALLTID  clio-docs, clio-vision, clio-transcribe, clio-narrate,
                  clio-audio-edit, clio-library, clio-emailfetch, clio-fetch

All logik finns i separata moduler:
...
# ── Verktygsregister ──────────────────────────────────────────────────────────
# Migrationsstatus anges i kommentar per verktyg.
# Ta bort ett verktyg härifrån EFTER att Odoo-addon är verifierat i drift.

TOOLS = [
    # ── 🖥️ CLI ALLTID — stannar i clio2 permanent ─────────────────────────────
    {
        "nr":     1,
        "name":   "clio-docs",
    ...
    },
    {
        "nr":           16,
        "name":         "clio-graph",
        "desc":         "Nätverksgraf — synka Odoo-relationer till Neo4j",
    ...
    # ── 🔜 RELEASE 1 → tas bort när clio_job-addon är verifierat ─────────────
    {
        "nr":           14,
        "name":         "clio-agent-job",
        "desc":         "Jobbsökning — förändringssignaler och karriärmöjligheter  [→ Odoo R1]",
        "script":       ROOT / "clio-agent-job" / "run.py",
        "status":       "active",
        "needs_folder": False,
        "custom_runner": True,
    },
    # ── 🔜 RELEASE 2 → tas bort när clio_obit-addon är verifierat ────────────
    {
        "nr":           11,
        "name":         "clio-agent-obit",
        "desc":         "Dödsannonsbevakning — daglig kontroll mot bevakningslista  [→ Odoo R2]",
        "script":       ROOT / "clio-agent-obit" / "run.py",
        "status":       "active",
        "needs_folder": False,
        "custom_runner": True,
    },
    # ── 🔜 RELEASE 3 → tas bort när clio_vigil-addon är verifierat ───────────
    {
        "nr":           15,
        "name":         "clio-vigil",
        "desc":         "Mediebevakning — RSS, YouTube, podcast → transkription → RAG  [→ Odoo R3]",
        "status":       "active",
        "needs_folder": False,
        "submenu": [
            {
                "nr":           1,
                "name":         "samla",
                "desc":         "Samla in nya objekt (RSS + YouTube) och filtrera",
            ...
            },
            {
                "nr":           2,
                "name":         "transkribera",
                "desc":         "Transkribera kön med Whisper",
                "script":       ROOT / "clio-vigil" / "main.py",
                "args":         ["--transcribe"],
                "needs_folder": False,
            },
            {
                "nr":           3,
                "name":         "summera",
                "desc":         "Sammanfatta transkript med Claude",
                "script":       ROOT / "clio-vigil" / "main.py",
                "args":         ["--summarize"],
                "needs_folder": False,
            },
            ...
            },
            {
                "nr":           5,
                "name":         "digest",
                "desc":         "Skicka daglig digest-mail",
                "script":       ROOT / "clio-vigil" / "main.py",
                "args":         ["--digest"],
                "needs_folder": False,
            },
            {
                "nr":           6,
                "name":         "statistik",
                "desc":         "Visa tillståndsöversikt per domän",
                "script":       ROOT / "clio-vigil" / "main.py",
                "args":         ["--stats"],
                "needs_folder": False,
            },
            {
                "nr":           7,
                "name":         "kö",
                "desc":         "Visa transkriptionskön (top 20)",
                "script":       ROOT / "clio-vigil" / "main.py",
                "args":         ["--list-queued"],
                "needs_folder": False,
            },
